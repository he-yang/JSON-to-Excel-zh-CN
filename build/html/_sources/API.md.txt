# 5. API（通过HTTPS POST请求将JSON转换为Excel）
...
- 标准方式（5.3节）：免费，采用标准转换规则。
- 专业方式（5.4节）：使用自定义转换规则，需要订阅有效的WTSolutions的JSON转Excel服务。更多详情请访问[定价](pricing.md)。
...
- JSON格式数据
- 指向JSON文件的URL
在本节中，您可以找到使用此 API 的标准方法，该方法是免费的。如果您想进行一些自定义转换，请参考 5.4 节 使用方法 - 专业。
...
本节 - 专业版适用于已购买 [JSON 转 Excel 服务订阅](pricing.md) 的用户。如果您尚未购买订阅，请参考 5.3 节 使用方法 - 标准版。
...
> 注意：
> - 提供 `data` 或 `url` 中的一个，不要同时提供。
> - 如果您想使用自定义转换设置，`options` 是必需的。如果您没有有效的专业版代码，请参考 5.3 节 使用方法 - 标准版。
